...
class Expression extends Base {
	...
	private function _setError($nCode, $sExpression) {
		...
		switch($nCode) {
			case self::ERROR_UNKNOWN: throw new ExpressionException("Unknown error.", $sExpression);
			case self::ERROR_INVALIDSTRING: throw new ExpressionException("Invalid string sequence.", $sExpression);
			case self::ERROR_INVALIDPARENTHESES: throw new ExpressionException("Invalid parentheses/braces.", $sExpression);
			case self::ERROR_INVALIDEXPRESSION: throw new ExpressionException("Invalid expression.", $sExpression);
			case self::ERROR_INVALIDWHITESPACE: throw new ExpressionException("Invalid whitespace.", $sExpression);
			case self::ERROR_INVALIDQUOTE: throw new ExpressionException("Invalid quote.", $sExpression);
			default: exit;
		}
	}
		...
		if(isset($sName)) {
			if(in_array($sName, $this->m_aKeywords)) {
				if(($sName === 'key' || $sName === 'index') && $sBase == 'null')
					$sReturn = ParserSettings::getCallKeywordIndex();
				else if(($sName === 'self' || $sName === 'this' || $sName === 'current' || $sName === 'value') && $sBase == 'null')
					$sReturn = ParserSettings::getCallKeywordCurrent();
				else if($sName === 'first')
					$sReturn = ParserSettings::getCallKeywordFirst($sBase);
				else if($sName === 'last')
					$sReturn = ParserSettings::getCallKeywordLast($sBase);
				else 
					$this->_setError(self::ERROR_INVALIDKEYWORD, $sName);
			}
			else if(empty($aParams)) {
				$sReturn = ParserSettings::getCallMember($sName, $sBase);
			}
			else {
				$sReturn = ParserSettings::getCallMethod($sName, 'array('.implode(',', $aParams).')', $sBase);
			}
		}
		foreach($aSlices as $sSlice) {
			$sReturn = ParserSettings::getCallSlice($sSlice, $sReturn);
		}
		return $sReturn;
	}
	...
	private function _parseValue($sExpression) {
		...
		// If those are not found, the parser will try to figure out if the value is a call.
			...
			// Look for array
				...
				return ParserSettings::getCallArray(array_map(array($this, '_parseExpression'), $aParams));
			}
